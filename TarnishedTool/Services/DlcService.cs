...
public class DlcService(IMemoryService memoryService) : IDlcService
{
    ...
    public void CheckDlc()
    {
        ...
        IsDlcAvailable = memoryService.Read<byte>(flags + (int)CsDlcImp.Flags.DlcCheck) == 1;
        PreOrderBaseGesture = memoryService.Read<byte>(flags + (int)CsDlcImp.Flags.DlcCheck) == 0;
        PreOrderDlcGesture = memoryService.Read<byte>(flags + (int)CsDlcImp.Flags.DlcCheck) == 2;
    }
